...
declare global {
  interface Window {
    zspark: {
      ...
      pickWorkspace: () => Promise<{ ok: boolean; path?: string; canceled?: boolean; error?: string }>
      switchWorkspace: (path: string) => Promise<{ ok: boolean; workspaceRoot?: string; attachmentDir?: string; error?: string; requiresRestart: boolean }>
      getRecentWorkspaces: () => Promise<WorkspaceInfo[]>
      detectTechStack: (path: string) => Promise<string>
...
function BridgeMissing() {
  ...
  return (
    <div className="bridge-missing">
      <div>
        <h1>{t('error.bridgeUnavailable')}</h1>
        <p>{t('error.bridgeHint')}</p>
      </div>
    </div>
  )
}
...
function DesktopApp() {
  ...
  const refreshRuntimeHost = async () => {
    try {
      ...
      if (info.recentWorkspaces) {
        const workspaces = await window.zspark.getRecentWorkspaces()
        setRecentWorkspaces(workspaces)
      }
      if (info.collapsedSections) {
        setCollapsedSections(info.collapsedSections)
      }
    } catch {}
  }
  ...
  const pickWorkspace = async () => {
    ...
    try {
      const result = await window.zspark.pickWorkspace()
      if (result.canceled) return
      if (!result.ok || !result.path) {
        toast('error', result.error ?? 'Could not pick workspace directory')
        return
      }
  ...
  const switchWorkspace = async (path: string) => {
    ...
    setWorkspaceBusy(true)
    try {
      const result = await window.zspark.switchWorkspace(path)
      if (!result.ok) {
  ...
  const getWorkspacePreview = (wsPath: string): string => {
    const normalized = wsPath.replace(/\\/g, '/').toLowerCase()
    const match = threads
      .filter((t) => {
        const tCwd = (t.cwd || '').replace(/\\/g, '/').toLowerCase()
        return tCwd === normalized
      })
      .sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0))[0]
    if (match?.preview) {
      return match.preview.trim()
    }
    return ''
  }
  ...
  const toggleSection = (section: keyof CollapsedSections) => {
    setCollapsedSections((prev) => {
      ...
      window.zspark.saveSettings({ collapsedSections: next } as any).catch(() => {})
      return next
    })
  }
    ...
    <div className="app">
      ...
      {showSettings && (
        <SettingsModal
          ...
          currentWorkspacePath={runtimeCwd ?? ''}
      ...
      {panel === 'search' && (
        <Drawer title={activeSharedWorkspace ? 'Search shared sessions' : 'Search threads'} onClose={() => setPanel(null)}>
          <input className="drawer-search" placeholder={t('search.placeholder')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          <div className="drawer-list">
            ...
            {visibleThreads.length === 0 && <div className="drawer-empty">{activeSharedWorkspace ? t('search.emptyShared') : t('search.empty')}</div>}
          </div>
        </Drawer>
      )}
      ...
      {panel === 'files' && (
        <Drawer title={activeSharedWorkspace ? t('nav.sharedArtifacts') : t('nav.workspaceFiles')} onClose={() => setPanel(null)}>
          <div className="shared-panel">
            ...
            {visibleWorkspaceFiles.length === 0 ? (
              <div className="drawer-empty">{activeSharedWorkspace ? 'No shared artifacts for this session yet.' : t('sidebar.noFiles')}</div>
            ) : (
              <div className="artifact-list">
          ...
          <div className="shared-panel">
            <div className={`shared-status ${enterprise?.signedIn ? 'connected' : ''}`}>
              ...
              <div>
                ...
                <span>{enterprise?.signedIn ? (enterprise.account?.username ?? t('workspace.enterpriseConnected')) : t('workspace.signInHint')}</span>
              </div>
            </div>
      ...
      {panel === 'history' && (
        <Drawer title={activeSharedWorkspace ? 'Shared session history' : 'Chat history'} onClose={() => setPanel(null)}>
          <div className="drawer-list">
            ...
            {visibleThreads.length === 0 && <div className="drawer-empty">{activeSharedWorkspace ? t('sidebar.noSharedSessionsShort') : t('sidebar.noHistory')}</div>}
          </div>
        </Drawer>
      )}

...
export function truncateActivityDetail(detail?: string, limit = 1800): string | undefined {
  ...
  return i18n.t('activity.outputTruncated', { limit }) + '\n\n' + trimmed.slice(-limit)
}
...
export function memoryCitationTitle(citation?: MemoryCitation | null): string {
  ...
  if (count > 0) return i18n.t('activity.memoryCited', { count })
  const threadCount = citation?.threadIds?.length ?? 0
  return threadCount > 0 ? i18n.t('activity.memoryFromThreads', { count: threadCount }) : i18n.t('activity.memoryReferenced')
}
...
export function replayActivityFromItem(item: any, fallbackStartedAt: number): Activity | undefined {
  ...
  if (item?.type === 'reasoning') {
    ...
    return { id: `replay-a-${id}`, kind: 'reasoning', title: i18n.t('activity.thought'), detail: detail || undefined, status: 'done', startedAt, endedAt }
  }
  ...
  if (item?.type === 'fileChange') {
    ...
    return {
      id: `replay-a-${id}`,
      kind: 'file',
      title: i18n.t('activity.filesChanged', { count: changes.length }),
  ...
  if (item?.type === 'contextCompaction') {
    return {
      id: `replay-a-${id}`,
      kind: 'memory',
      title: i18n.t('activity.compactedContext'),
      detail: i18n.t('activity.compactedDetail'),
      status: 'done',
      startedAt,
      endedAt
    }
